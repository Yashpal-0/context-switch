...
timer_interrupt_arrived = false;
timer_interrupt_running = true;

read_interrupt_arrived = false;
read_interrupt_running = true;

exit_interrupt_arrived = false;
exit_interrupt_running = true;

exit2_interrupt_arrived = false;
exit2_interrupt_running = true;

type_of_interrupt = "";

/* *************
    Declaring the process instructions and regsiters as dictionaries
****************/
...
function load_process_A() {
    const process_A = document.getElementById("process");
    process_A.innerHTML = "";
...
function load_process_B() {
    const process_B = document.getElementById("process");
    process_B.innerHTML = "";
...
function setUp_Hardware() {
    ...
    const hardwareInstructions_u_to_k = Hardware_userToKernelMode(process);

    for (let i = 0; i < hardwareInstructions_u_to_k.length; i++) {
        const tr = document.createElement("tr");
        const td1 = document.createElement("td");
    ...
    document.getElementById("hardware").innerHTML = "HARDWARE: " + type_of_interrupt;
...
function start_hardware() {
    ...
    const hardware_button = document.getElementById("hardware_buttons");
...
function start() {
    ...
    const tbody_cpu = document.createElement("tbody");
    tbody_cpu.insertRow(0);
    tbody_cpu.rows[0].insertCell(0).innerHTML = "PC: ";
    tbody_cpu.rows[0].insertCell(1).innerHTML = "1";

    tbody_cpu.insertRow(1);
    tbody_cpu.rows[1].insertCell(0).innerHTML = "Mode: ";
    tbody_cpu.rows[1].insertCell(1).innerHTML = "User";

    tbody_cpu.insertRow(2);
    tbody_cpu.rows[2].insertCell(0).innerHTML = "Registers: ";
    tbody_cpu.rows[2].insertCell(1).innerHTML = get_cpu_regs();
...
function process_A_next() {
    ...
    if (prog_A_ip == 10 && exit_interrupt_arrived == false) {
        ...
        window.alert("The process has to perform an implicit exit to finish it's execution.");
    ...
    // Update CPU's PC
    if (prog_A_ip == 10) {
        cpu.rows[0].cells[1].innerHTML = prog_A_ip;
    }
    else {
        cpu.rows[0].cells[1].innerHTML = prog_A_ip+1;
    }
...
function process_B_next() {
    ...
    if (prog_B_ip == 18 && exit2_interrupt_arrived == false) {
        ...
        window.alert("The program had enountered an implicit exit. Hardware will take over the control");
    ...
    // Update CPU's PC
    if (prog_B_ip == 18) {
        cpu.rows[0].cells[1].innerHTML = prog_B_ip;
    }
    else {
        cpu.rows[0].cells[1].innerHTML = prog_B_ip+1;
    }
    // put the registers in the register set in the CPU according to the instruction
    cpu.rows[2].cells[1].innerHTML = get_cpu_regs();
}
...
function process_A_previous() {
    ...
    prog_A_ip -= 1;
    // Update CPU's PC
    document.getElementById("cpu_section").rows[0].cells[1].innerHTML = prog_A_ip + 1;
    // put the registers in the register set in the CPU according to the instruction
    document.getElementById("cpu_section").rows[2].cells[1].innerHTML = process_A_regSet[prog_A_ip].value;
    // if the previous instruction is the first instruction, then disable the previous button
    process_A = document.getElementById("process");
    if (prog_A_ip == 0) {
        document.getElementById("prev_process").disabled = true;
    }
    // if the next button is disabled, enable it
    if (prog_A_ip == Program_A_instructions.length - 2) {
...
function process_B_previous() {
    ...
    prog_B_ip -= 1;
    // Update CPU's PC
    document.getElementById("cpu_section").rows[0].cells[1].innerHTML = prog_B_ip + 1;
    // put the registers in the register set in the CPU according to the instruction
    document.getElementById("cpu_section").rows[2].cells[1].innerHTML = process_B_regSet[prog_B_ip].value;
    // if the previous instruction is the first instruction, then disable the previous button
    process_B = document.getElementById("process");
    if (prog_B_ip == 0) {
        document.getElementById("prev_process").disabled = true;
    }
    // if the next button is disabled, enable it
    if (prog_B_ip == Program_B_instructions.length - 2) {
        document.getElementById("next_process").disabled = false;
    }
    // update the highlighted instruction
    process_B.rows[prog_B_ip].style.backgroundColor = "yellow";
...
function load_memory_Table() {
    const memoryTable = document.getElementById("memTable");
...
function hardware_next() {
    ...
    if (hardwarePointer == 3) {
        document.getElementById("next_hardware_instruction").disabled = true;
    }
    
    hardware_instructions = document.getElementById("interrupt");
...
function hardware_previous() {
    ...
    if (hardwarePointer == 2) {
        document.getElementById("next_hardware_instruction").disabled = false;
    }
}

function loadTheHandlers() {
    handlers = document.getElementById("handlers");
    handlers.innerHTML = "";

    timerTrapHandler = document.createElement("button");
    ...
    readTrapHandler = document.createElement("button");
    ...
    exitTrapHandler = document.createElement("button");
...
function load_exit_handler() {
    ...
    const hardware_buttons = document.getElementById("hardware_buttons");
    hardware_buttons.innerHTML = "";

    const start_hardware_button = document.createElement("button");
    ...
    const p_hardware = document.getElementById("hardware");
    p_hardware.innerHTML = "HARDWARE";

    // Get the reference to the "kernel" table
    const kernelTable = document.getElementById("kernel");
...
function execute() {
    ...
    setTimeout(function () {
        ...
        kernelSection.innerHTML =
            "<tr><td>Executed the trap Handler</td></tr><tr><td>Please do the contect switch ... </td></tr>";
...
function contextSwitch() {
    if (k_trap_execute == 1) {
        ...
        if (exit2_interrupt_arrived == true) {
            ...
            document.getElementById("next_kernel_instruction").disabled = true; 
        }
        else {
...
function updatePCB_B() {
    const pcb_B_table = document.getElementById("pcbB");
    pcb_B_table.rows[1].cells[1].innerHTML = prog_B_ip+1;
    if (prog_B_ip == 18) {
        pcb_B_table.rows[1].cells[1].innerHTML = prog_B_ip;
    }
...
function updatePCB_A() {
    const pcb_A_table = document.getElementById("pcbA");
    pcb_A_table.rows[1].cells[1].innerHTML = prog_A_ip+1;
    if (prog_A_ip == 10) {
        pcb_A_table.rows[1].cells[1].innerHTML = prog_A_ip;
    }
...
function executeNextKernelInstruction() {
    ...
    if (kernelPointer == kernelTable.rows.length - 1) {
        ...
        kernelButtons = document.getElementById("kernel_buttons");
        kernelButtons.innerHTML = "";
    ...
    kernelTable.rows[kernelPointer - 1].style.backgroundColor = "white";
    kernelTable.rows[kernelPointer].style.backgroundColor = "yellow";
}

function load_Hardware_k_to_u() {
    const hardwareTable = document.getElementById("interrupt");
    ...
    hardwarePointer = 0;
    hardwareTable.rows[0].style.backgroundColor = "yellow";
    hardwareTable.rows[0].style.fontWeight = "bold";
...
function hardware_next_k_to_u() {
    ...
    if (hardwarePointer == 2) {
        ...
        const process_table = document.getElementById("process");

        process_table.rows[process_ip()].style.backgroundColor = "yellow";
        process_table.rows[process_ip()].style.fontWeight = "bold";
        ...
        // }
        // else {
        //     process_table.rows[process_ip()+1].style.backgroundColor = "yellow";
        //     process_table.rows[process_ip()+1].style.fontWeight = "bold";
        // }

        document.getElementById("prev_hardware_instruction").disabled = true;
    ...
    hardwareTable.rows[hardwarePointer].style.backgroundColor = "yellow";
    hardwareTable.rows[hardwarePointer].style.fontWeight = "bold";
}
